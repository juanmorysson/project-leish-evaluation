...
# --- ARQUIVOS ---
output_file = "scores_judge.csv"
input_file = "respostas_melhor_diretriz_corrigido.csv"
coerencia_file = "scores_coerencia.csv"
...
# 1. Preparação e Lógica de Resume
df_input = pd.read_csv(input_file)
df_coerencias = pd.read_csv(coerencia_file)

pacientes_processados = set()
if os.path.exists(output_file):
    try:
        df_existente = pd.read_csv(output_file)
        # Pegamos os IDs únicos que já constam no arquivo de saída
        pacientes_processados = set(df_existente['paciente'].unique())
        print(f"Retomando execução. {len(pacientes_processados)} pacientes já processados.")
    except Exception as e:
        print(f"Erro ao ler arquivo existente, começando do zero: {e}")
...
for _, row in tqdm(df_input.iterrows(), total=len(df_input)):
    ...
    # Pula se o paciente já estiver no CSV de saída
    if paciente_id in pacientes_processados:
        continue
    #print("")
    #print(paciente_id)
    for tipo in ['a', 'b', 'c']:
